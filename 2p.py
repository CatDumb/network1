...
CURRENT_ALIAS = 'Your messages will appear here'
...
def on_new_connection():
    while True:
        ...
        listening_thread = threading.Thread(target=receive_message, args=(conn, addr, "in",), daemon=True)
...
def connect_to_others(raw_foreign_addr):
    ...
    receiving_thread = threading.Thread(target=receive_message, args=(new_client_socket, foreign_addr, "out",), daemon=True)
...
def receive_message(conn, addr, in_or_out):
    while True:
        ...
        if client_msg_list[0] == (protocols.INTRODUCE):
            ...
            client_message_content = client_msg_list[1]
            window['-CHATOUTPUT-'+sg.WRITE_ONLY_KEY].print(f'[{addr}]: ' + client_message_content)
...
message_layout = [
    [sg.Text(CURRENT_ALIAS, key='-CURRENTALIAS-')],
...
chat_layout = [
    [
        sg.Column(
            onlinePeersLayout, element_justification="c", pad=(20, 20)
        ),
...
while True:
    ...
    if event == "Refresh":
        ...
        if selection:
            ...
            window['-CURRENTALIAS-'].update(alias_to_connect)
